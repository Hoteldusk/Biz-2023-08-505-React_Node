/**
 * fileReader에 의해서 load가 완료되면
 * 파일 정보를 return 해주기를 바라는 코드
 *
 * 이 코드에서 return fe.target.result가 가장 마지막에
 * 실행되고, 그 결과를 return 해주기를 바라고 있다.
 * 하지만 비동기 특성상 2번 항목인 fileReader.read...() 함수가
 * 실행된 다음 filePreview 함수는 종료되어 버린다.
 * 결국 return null을 실행하는 결과만 만들어 낸다.
 *
 * 최신 JS 에서는 async, await을 사용하여 비동기를 동기 방식으로
 * 사용할 수 있다.
 * 하지만 상당부분의 JS 코드에서 async await을 사용하지 못하는 경우가 있다.
 *
 * 이 때는 코드 전체를 Promise 객체로 생성하여 동기를 보장해 주어야 한다.
 */
...
/**
 * input tag에 의해 얻어진 files 데이터는 배열이지만 배열이 아니다
 * 이러한 배열을 유사배열 이라고 한다.
