/**
 * web browser 의 indexedDB 를 핸들링 하기 위한 react 3rd party
 */
...
/**
 * 코딩에서 magic literal(매직스트링, 매직넘버) 사용 지양
 * 문자열로 생성된 값을 참조하여 어떤 문제를 해결하는 것
 * if(name === '홍길동') 이런거 하지말것
 *
 * nickname = '홍길동';
 *
 * const SAMPLE_NAME = '홍길동';
 * nickname = SAMPLE_NAME;
 * if(name === SAMPLE_NAME) 이렇게 할것
 */
...
// id 값을 매개변수로 받아서 리스트 중 id 값에 해당하는 한개의 아이템을 return
export const getBucket = async (id) => {
  ...
  // bucketList 에서 데이터를 find 했는데, 결과 값이 null 이거나 undefined 인 경우도 있다
